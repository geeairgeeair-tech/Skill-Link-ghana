ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS service_area_id uuid REFERENCES public.service_areas(id);
CREATE INDEX IF NOT EXISTS bookings_service_area_id_idx ON public.bookings(service_area_id);

DROP FUNCTION IF EXISTS public.customer_create_booking(uuid, uuid, uuid, uuid, text, text, text, timestamptz, integer, integer, text, numeric, numeric, jsonb);
...
CREATE OR REPLACE FUNCTION public.customer_create_booking(
  ...
  _service_area_id uuid DEFAULT NULL
)
...
BEGIN
  ...
  -- Canonical general service area must be active AND covered by this professional.
  ...
  BEGIN
    ...
    INSERT INTO public.bookings (
      ...
      trim(_description), trim(_address), COALESCE(_area_name, trim(_service_area)), _service_area_id, _scheduled_at,
...
REVOKE ALL ON FUNCTION public.customer_create_booking(uuid, uuid, uuid, uuid, text, text, text, timestamptz, integer, integer, text, numeric, numeric, jsonb, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.customer_create_booking(uuid, uuid, uuid, uuid, text, text, text, timestamptz, integer, integer, text, numeric, numeric, jsonb, uuid) TO authenticated;
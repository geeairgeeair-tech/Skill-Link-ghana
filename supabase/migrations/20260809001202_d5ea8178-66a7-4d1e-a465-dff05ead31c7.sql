DROP FUNCTION IF EXISTS public.customer_update_job_request(uuid, text, text, uuid, integer, text, timestamptz, text, text, text, text, text, text, text);
...
CREATE OR REPLACE FUNCTION public.customer_update_job_request(
  ...
  _urgency text, _preferred_at timestamp with time zone, _city text, _address text,
...
BEGIN
  ...
  IF _media IS NOT NULL THEN
    IF jsonb_typeof(_media) <> 'array' THEN RAISE EXCEPTION 'Invalid media'; END IF;
    IF jsonb_array_length(_media) > 6 THEN RAISE EXCEPTION 'Maximum 6 files per job'; END IF;
  END IF;
